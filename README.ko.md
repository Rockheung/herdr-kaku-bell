...
- **대상 판별**: 자식 `ssh` 프로세스도 같은 tty 를 쓰고 명령줄에 `ssh://target` 을
  담고 있다. `argv[0]` 이 `herdr` 이고 `argv[1]` 이 `--remote` 인 것만 센다. 대상
  문자열은 손대지 않고 그대로 넘긴다 — herdr 가 그 값을 `ssh` 에 그대로 주기 때문에
  config 별칭, `user@host`, `ssh://` URI, 포트가 붙은 형태가 모두 온다.
...
- **예외 내성**: 한 주기가 실패해도 기록만 남기고 다음 주기를 계속 돈다. 감시자가 조용히
  죽으면 알림이 그냥 오지 않고, 그 사실을 알 방법도 없다.
